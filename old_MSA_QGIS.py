 ...
 ***************************************************************************/
"""
""" This version of the file was saved separately to serve as an easy reference during 
significant changes to incorporate subprocessing and multiprocessing"""
...
#** RUN MSA (QgsTask) # TODO does not work... for multiprocessing
    ...
    def testProcess(self, wait_time=2):
        """ testcase for testing multiprocessing"""
        QgsMessageLog.logMessage('started process testProcess', 'multitask', Qgis.Info)
        time.sleep(wait_time)
        QgsMessageLog.logMessage('finished process testProcess', 'multitask', Qgis.Info)
        return wait_time
...
### FULL MSA multiprocessed

            # Each iteration gets its own process
            # Code should not continue until all processes are finished

#             #test case
#             processes = []
#             for _ in range(number_of_iters):
#                 QgsMessageLog.logMessage(f'{sys.executable} sys.executable2', 'multitask', Qgis.Info)
#                 process = multiprocessing.Process(target=self.testProcess)
#                 process.start()
...
#
# ## Multiprocess using subprocesses https://gis.stackexchange.com/questions/239098/multiprocessing-in-a-qgis-plugin
#             # TODO: Method originally for QGIS2, surely there must be something better out there?
#             #write the file that contains all the required input
#
#
#             import subprocess
#             basepath = dirname(abspath(__file__))
#             file_to_run = basepath + "/MSA_QGIS_fullMSA_multiprocess"
#             subprocess.Popen(["python", file_to_run, argsFile])
#
